use jseries_core::{AssembledMessage, DecodeContext, Decoder, FieldStatus, Value, normalize_word};
...
fn loads_and_decodes_repository_example() {
    ...
    let message = AssembledMessage::new(vec![normalize_word(0x1c94).unwrap()]).unwrap();
    let decoded = decoder
        .decode(
            "EXAMPLE-70",
            &message,
